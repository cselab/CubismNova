// File       : Profiler.h
// Created    : Mon Apr 01 2019 04:48:11 PM (+0200)
...
struct ProfileSummaryItem {
    ...
    ProfileSummaryItem(std::string sName_,
                       double dTime_,
                       int nMoney_,
                       int nSamples_)
...
public:
    ...
    void printSummary(FILE *outFile = NULL) const
    {
        ...
        for (std::vector<ProfileSummaryItem>::const_iterator it = v.begin();
            ...
            printf("[%15s]: \t%02.0f-%02.0f%%\t%03.3e (%03.3e) s\t%03.3f "
                   "(%03.3f) s\t(%d samples)\n",
                   item.sName.data(),
                   100 * item.dTime / dTotalTime,
                   100 * (item.dTime - item.nSamples * 1.3e-6) / dTotalTime2,
                   avgTime,
                   avgTime - 1.30e-6,
                   item.dTime,
                   item.dTime - item.nSamples * 1.30e-6,
                   item.nSamples);
            if (outFile)
                fprintf(outFile,
                        "[%15s]: \t%02.2f%%\t%03.3f s\t(%d samples)\n",
    ...
    createSummary(bool bSkipIrrelevantEntries = true) const
    {
        ...
        for (std::map<std::string, ProfileAgent *>::const_iterator it =
            ...
            if (!bSkipIrrelevantEntries || agent.m_dAccumulatedTime > 1e-3)
                result.push_back(ProfileSummaryItem(it->first,
                                                    agent.m_dAccumulatedTime,
                                                    agent.m_nMoney,
