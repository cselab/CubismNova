 ...
 *
 * @rst
 * ``std::sqrt()`` is overloaded for integral types ``T`` and returns a double.
 * This causes implicit casts if the return type of a function is ``DataType``
 * which may be an integral type.  This version forces a link-time error for any
 * unsafe types ``T``.
 * @endrst
 */
template <typename T>
